...
public class Advisor extends User {
	...
	Advisor() {// constructor method called Room(). It is used to initialize
				// states of the class
	...
	// This is a void method which first finds an empty slot which is the
	// location of the array isBooked
	void makeBooking() {

		int emptySlot = -1; /*
							 * this variable is initialised so if it does not
							 * change we know code below has not found an
							 * emptyslot
							 */
		...
		if (emptySlot != -1) { // emptyslot -1 means nothing is available
			...
			String[] booking = { ProcessLogIn.studentUsername, ProcessLogIn.currentAdvisorID,
					ProcessLogIn.currentAdvisorFirstName, ProcessLogIn.currentAdvisorLastName,
					getStudentSlot()[emptySlot], ProcessLogIn.studentFullName };
			ProcessLogIn.bookingList.add(booking);
			isBooked[emptySlot] = "1";// makes the booking for the first
										// available slot
		} // If
	...
	int findEmptySlot() { // this is a return method
		int emptySlot = -1; // if the returned value is -1, there are no time
							// slots available
		for (int i = 0; i < isBooked.length; i++) {// used to go through the
													// array
			if (isBooked[i] == "0") {// finds an empty slot
				emptySlot = i;// assign the location of the array to emptySlot
								// variable
				break;
			} // if
		} // for
